...
struct SharedVolumeTextureData {
    std::size_t nx{0};
    std::size_t ny{0};
    std::size_t nz{0};
...
public:
    ...
    void set_live_volume(SharedVolumeTextureData data, QString field_name);
    void clear_live_volume();
    ...
    void set_live_opacity(float opacity);
    void set_live_threshold(float threshold);
...
private:
    ...
    unsigned int live_texture_{0};
    std::array<std::size_t, 3> live_texture_dimensions_{0, 0, 0};
    ...
    float live_opacity_{0.88F};
    float live_threshold_{0.04F};
};
